...
- **Dual-protocol DSH compatibility (0.0.12)**: the client auto-answer supports both approval delivery protocols — DSH 0.1.1-rc.2 (`snapshot.pending` + `wait.respond`) and 0.1.2-alpha.1 (`uiSession.pendingInteractions` + `PendingApproval.answer`) — selected by runtime capability (feature-detect); 0.0.15 will keep the new protocol only.
...
| `trustedDshSubpaths` | [] | Host-only key: DSH_HOME subtrees an Auto session may write (array of absolute paths). Empty by default = the whole DSH_HOME tree stays hard-denied (consistently across `edit`/`write`/`apply_patch`/`str_replace_editor`); a listed subtree gets the same allow as the plugin's own development zone. Patch/YAML only. Entries are dropped with a warning when they are not absolute, sit outside DSH_HOME, name DSH_HOME itself, cover `sessions`/`plugins`/`dsh-web-token.txt`/`dsh-web-url.txt`/`credentials*`, or normalize into a critical tree. **Know before enabling**: skill files are injected into the agent's context as instructions, so opening `skills` lets the agent durably rewrite its own constraints. The plugin's runtime-state hard-deny (history/audit/learning…) is orthogonal and unaffected |
