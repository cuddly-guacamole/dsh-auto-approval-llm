...
| `protectedAutoReview` | false | Unlocks the protected category clamp (off by default = fail closed): an unconfigured protected call then becomes an ordinary ask the reviewer can answer (locked, that ask is pinned to reject and nobody can answer it); set the category to `auto` explicitly to auto-allow it. **Wider than the name suggests**: beyond workspace secret files and protected metadata (`.env` / `.npmrc` / `.git`, …) it also covers **reads** of credential trees (`~/.ssh`, `~/.aws`, …), which are protected asks in the first place, so the reviewer can answer those too. Their **writes** stay hard-denied either way |
