...
test('the client long-polls instead of waking on a fixed cadence', () => {
  ...
  assert.ok(client.includes('dsa-sessionSplit'), 'the session control must render as a split control')
  assert.ok(client.includes('dsa-sessionDivider'), 'the split control must show its divider')
  assert.ok(client.includes('chipLabel(chipState(activeRecord, Date.now(), isLinkDown()))'), 'the control label must read the display store')
