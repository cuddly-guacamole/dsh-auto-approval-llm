...
| `protectedAutoReview` | false | 受保护类别解锁开关（默认关=fail-closed）：解除 `protected` 的锁定钳制。解除后**未显式配置**的受保护调用变为普通询问（评审器可作答；锁定态那条询问无人能答且倒计时恒拒），把该类别显式设为 `auto` 才会自动放行。**解锁面较宽**：protected 涵盖工作区敏感文件与受保护元数据（`.env`/`.npmrc`/`.git` 等）**以及凭据树的读取**（`~/.ssh`/`~/.aws` 等——读取本就是 protected 询问，只有**写入**是硬拒且不受本键影响）。删除/磁盘不受影响仍锁 ask |
