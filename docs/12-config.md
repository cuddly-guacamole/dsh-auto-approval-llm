...
| `protectedAutoReview` | false | 受保护类别解锁开关（默认关=fail-closed）：解除 `protected` 的锁定钳制。解除后**未显式配置**的受保护调用不再被恒拒倒计时钉死，而落常驻人工询问——类别询问在 pre-execute 即返回，**评审器仍不会被问到**，须人工作答；把该类别显式设为 `auto` 才会自动放行。**凭据读取地板**：敏感文件名/目录与关键路径的**读取**（含写头读源）被结构化判定为 `credentialRead` 并保持锁定，本键只解锁非凭据的工作区元数据（`.git`/`.vscode` 等）；地板按类别层视角生效，opaque 行与 shell junction 逃逸为已登记残余面（见 docs/17）；凭据树的**写入**本就硬拒且不受本键影响。删除/磁盘不受影响仍锁 ask |
