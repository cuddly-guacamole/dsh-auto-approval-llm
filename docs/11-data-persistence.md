...
- **读**（<span class="lnum">runtime-paths.ts:LresolveRuntimeReadPath</span>）：规范目录可用时优先规范副本；该副本不存在时回退到**插件根目录**的旧文件（已发布版本写入的位置）。**写入降级后改为「较新者胜」，并按文件类型各带一道校验**——新记录写向旧根，降级会把规范内容播种过去。**仅追加型**：先校验「一份是另一份的字节前缀」再覆盖，校验不过判分叉 → 读规范副本、保留两份并告警（需人工作结）。**覆盖型**（learning / review-mode）：整份快照且会合法收缩，故**不做包含校验、较新者胜**；若较新者不再包含规范副本的全部条目，被取代的副本先另存 `<name>.superseded`（该槽位已被占用时**不采纳**、转告警待人工合并，以免覆盖上一代备份）再搬回并告警。规范目录恢复可用后的下一次启动搬回较新的旧根副本（`reconcileRuntimeCopies`，同规则）。两处都没有时返回规范路径，调用方报出的位置不会是历史位置。
...
- **目录存在但拒绝写入**（与上一条不同的失效形状，两条机制共同闭合）：`mkdirSync(..., {recursive:true})` 对**已存在**的目录返回成功，所以「能不能建出目录」回答不了「能不能写进去」——该目录以其他账号创建、只读挂载、或运行态文件位置被一个同名目录占位时，过去会判为可用、每次 append 失败、审计闸把**全部裁决**转拒。现在由**启动期实写探针** `probeRuntimeDirWritable()`（<span class="lnum">runtime-paths.ts:LprobeRuntimeDirWritable</span>，在 `apply()` 里实写一次探针文件并删除，失败即降级）与**写失败阶梯** `appendRuntimeLine` / `writeRuntimeAtomic`（<span class="lnum">runtime-paths.ts:LappendRuntimeLine</span>）闭合，两者都**不引入 check→write 的 TOCTOU 窗口**（**写入本身就是可写性判据**），并共用同一套错误码分级（见下条）：同路径重试只发生在重试无害的错误上，且只有「该位置拒绝写入」的错误码才降级——探针不因一次瞬时占用就搬走六个文件。降级是**粘滞**的（`stateDirWriteDenied` 与目录可用性缓存分开：`mkdirSync` 对已存在目录恒真，清缓存拦不住它），已被判定拒绝写入的目录不会在每次追加时重试。
- **降级期读链跟随写链（不变量，附两条刻意例外）**：降级后 `resolveRuntimeReadPath` 取**经校验的较新者**——新记录写向旧根，而旧根副本在降级时已由规范副本播种，所以正常情况下「较新」即「超集」。读链不跟随写链就是「脑裂」：进程会一边往旧根追加、一边从冻结的规范副本读回，于是每次持久化（每会话评审模式、学习条目、历史）都在下次加载时静默丢失。反向同样成立：迁移**从不删除**旧副本，所以「陈旧的旧根快照」是迁移后的常态，降级时若不把规范内容播种过去，读链一翻转就会丢掉迁移以来的全部记录。**例外一（仅追加型）**：只有两份副本确实有序（一份是另一份的字节前缀）时时间戳才被采信；校验发现分叉则读链**固定读规范副本**（审计证据基座）并告警，一份已经校验过的超集结论**优先于时间戳**（规范副本 mtime 被外部刷新也不能把读抢回去）。**例外二（覆盖型）**：`learning.json` / `review-mode.json` 是整份快照且**会合法收缩**（review-mode 不落默认 `smart`；learning 有 30 天 TTL、100 条上限、用户撤销），所以既不能做并集（会把用户刚撤销的学习条目与刚改回的会话模式复活），也不能要求包含（会丢掉降级窗口里的全部改动）——**较新者胜**，但若它不再包含规范副本的全部条目，被取代的副本会另存为 `<name>.superseded` 并告警一次。
...
- `trusted-intents`：**授权证据的来源构成**——分类器边界每次实际收到的 trusted intents **条数与各来源计数**（`user-message` / `question-answer`），按会话去重、来源混合变化才追加，**只记来源不记用户原文**。为什么需要：用户经 `ask_user_question` 勾选做出的授权若没能进入 trusted intents，分类器只会看到"无授权"并拒绝——没有报错、没有告警，审计行与普通拒绝**无法区分**；该通道曾静默失效（`tool/result` 的嵌套 payload 形状与提取器不符），故留一条可自动观测的痕迹。
