...
- **降级期读链跟随写链（不变量）**：一旦降级，`resolveRuntimeReadPath` 取**两者中较新的可读副本**——新记录写向旧根，而旧根副本在降级时已由规范副本播种，所以「较新」即「超集」。读链不跟随写链就是「脑裂」：进程会一边往旧根追加、一边从冻结的规范副本读回，于是每次持久化（每会话评审模式、学习条目、历史）都在下次加载时静默丢失。反向同样成立：迁移**从不删除**旧副本，所以「陈旧的旧根快照」是迁移后的常态，降级时若不把规范内容播种过去，读链一翻转就会丢掉迁移以来的全部记录（这正是第二轮独立复核报出的阻断项，见 `.agents/progress/2026-09-11-runtime回退与shell逃逸复检批.md` §五）。**恢复方向**：规范目录恢复可用后的下一次启动（`probeRuntimeDirWritable()` 成功时）会把较新的旧根副本搬回规范目录，否则降级窗口的记录会在规范副本再一次变新后被永久孤立。**注意**：offline 脚本（`audit-query` / `friction-report`）仍是「规范优先」的独立内联实现，降级期请以一次性的 `console.warn` 所指位置为准。
