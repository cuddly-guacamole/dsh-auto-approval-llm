...
- **读**（<span class="lnum">runtime-paths.ts:LresolveRuntimeReadPath</span>）：规范目录可用时优先规范副本；该副本不存在时回退到**插件根目录**的旧文件（已发布版本写入的位置）。**写入降级后改为「经校验的较新者胜」**——新记录写向旧根；降级会把规范内容播种过去，且只有当「校验确认一份是另一份的前缀/包含另一份」时时间戳才被采信：仅追加型比字节前缀，覆盖型比条目包含，均**先校验后写入**。校验发现分叉则读规范副本、保留两份并告警（需人工作结）；规范目录恢复可用后的下一次启动把较新的旧根副本搬回（`reconcileRuntimeCopies`，同样先校验）。两处都没有时返回规范路径，调用方报出的位置不会是历史位置。
