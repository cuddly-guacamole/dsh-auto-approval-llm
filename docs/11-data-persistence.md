...
- **目录存在但拒绝写入**（与上一条不同的失效形状）：`mkdirSync(..., {recursive:true})` 对**已存在**的目录返回成功，所以"能不能建出目录"回答不了"能不能写进去"——该目录以其他账号创建、只读挂载、或运行态文件位置被一个同名目录占位时，过去会判为可用、每次 append 失败、审计闸把**全部裁决**转拒。现在两条机制共同闭合，且都不引入 check→write 的 TOCTOU 窗口（**写入本身就是可写性判据**）：`probeRuntimeDirWritable()`（<span class="lnum">runtime-paths.ts:LprobeRuntimeDirWritable</span>）在 `apply()` 里启动期实写一次探针文件并删除，失败即降级；`appendRuntimeLine` / `writeRuntimeAtomic`（<span class="lnum">runtime-paths.ts:LappendRuntimeLine</span>）在写失败时**同一路径重试一次**（吸收 Windows 上的瞬时 rename 竞争），仍失败且错误码属「该位置拒绝写入」白名单（`EACCES` / `EPERM` / `EROFS` / `EISDIR` / `ENOTDIR`，均为**打开目标时**抛出、重试不会重复或截断记录）才粘滞降级到插件根目录并告警。`ENOSPC` / `EIO` / `EMFILE` 这类**字节已经动过**的错误刻意不在白名单：它们继续 fail-closed，绝不静默换位置。降级是粘滞的，已被判定拒绝写入的目录不会在每次追加时重试。
