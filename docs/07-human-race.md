...
枪声一响，三方赛跑：**人**（点官方面板按钮）、**宿主倒计时**（唯一权威作者）、**LLM 结论**（scope 内可抢占）。`raceHumanDecision`（<span class="lnum">decision.ts:L115-169</span>）是仲裁器：
...
## 7.1　诚实来源标注 `approvalSource` <span class="lnum">decision.ts:L186-208</span> —— 谁决定，就标谁
