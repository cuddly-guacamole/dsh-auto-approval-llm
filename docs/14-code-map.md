...
│    ├─ runtime-paths.ts 492  运行态文件唯一路径 owner（`<DSH_HOME>/auto-approval-llm/` 规范位置、包根回退读、目录不可用/拒绝写入回退写、仅追加型文件原子前搬、启动期写探针与写失败阶梯；迁移代码待退役）
│    ├─ shell.ts         1921 bash/pwsh 词法分解 + 整行熔断 + 逐段静态分类（最大单文件）
│    ├─ symlink.ts       176  符号链接创建与目标校验
