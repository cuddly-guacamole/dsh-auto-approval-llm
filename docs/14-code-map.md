...
```text
src/
├─ index.ts              宿主编排：apply()、四挂点接线、16 路由、命令、评审器、审计、学习接线  5917 行
...
│    ├─ category.ts      916  12 类三态开关层：归类/优先级合并/指令钳制/信任目录模式
│    ├─ classifier.ts    100  预分类提示词、参数脱敏、严格响应解析
│    ├─ constants.ts     141  数值默认唯一事实源（倒计时/熔断/截断/学习族阈值）
│    ├─ decision.ts      931  纯决策函数：评审解析、人机竞速、来源标注、熔断、静态名单、host-only 键
...
│    ├─ paths.ts         447  路径规范化、受保护/关键路径判定、受保护读取例外（Git ref 元数据）、运行态文件名单
...
│    ├─ shell.ts         3513 bash/pwsh 词法分解 + 整行熔断 + 逐段静态分类（最大单文件）
...
└─ 合计 205 个 tests/*.test.mjs（node --test 全绿基线）
scripts/
