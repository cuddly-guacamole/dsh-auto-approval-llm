...
```text
src/
├─ index.ts              宿主编排：apply()、四挂点接线、16 路由、命令、评审器、审计、学习接线  5940 行
...
└─ client/
     ├─ index.ts         React 客户端主体 2972 行（设置卡 8 子卡/面板增强/应答 watcher 装配/浮动按钮/CSS）
...
└─ 合计 185 个 tests/*.test.mjs（node --test 全绿基线）
scripts/
