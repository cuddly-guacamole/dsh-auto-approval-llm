...
- **读**优先规范目录；缺失时回退到插件根目录的旧文件（已发布版本写入的位置）。规范目录可用时以规范副本为准；**写入降级后改以两者中较新的一份为准**（降级会把规范内容播种到旧根，恢复时再搬回规范目录）。时间戳仅在两份副本经校验存在包含关系时才被采信（仅追加型比前缀、覆盖型比条目，且先校验后写入），校验不过则保留规范副本并告警。
