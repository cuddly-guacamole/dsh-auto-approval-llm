...
- **读**优先规范目录；缺失时回退到插件根目录的旧文件（已发布版本写入的位置）。规范目录可用时以规范副本为准；**写入降级后改以两者中较新的一份为准**（降级会把规范内容播种到旧根，恢复时再搬回规范目录）。仅追加型先校验字节前缀再覆盖，校验不过则保留规范副本并告警；覆盖型是整份快照且会合法收缩，故较新者胜，但被取代的副本会另存 `.superseded` 并告警。
