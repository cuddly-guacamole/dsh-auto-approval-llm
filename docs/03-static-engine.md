...
| `paths.ts` <span class="lnum">paths.ts#</span> | 440 | 路径规范化（Windows 命名空间/NT 别名折叠、~ 展开、win32 小写）、受保护/关键路径判定、运行态文件名单 |
| `shell.ts` <span class="lnum">shell.ts#</span> | 3503 | Bash/PowerShell 词法分解（sticky 正则状态机）＋ 整行熔断 ＋ 逐段静态分类 |
| `policy.ts` <span class="lnum">policy.ts#</span> | 653 | 每次工具调用的确定性第一遍分类 `assessTool`（推断型、保留类型检查） |
...
| `decision.ts` <span class="lnum">decision.ts#</span> | 931 | 纯决策函数：评审解析、人机竞速、来源标注、熔断状态机、静态名单 |
...
flowchart TD
    ...
    A7 --> A8["⑧ SESSION_STATE_TOOLS（13：ask_user_question/todo_write/get_goal/… + present/cordis_define/cordis_stop/cordis_undefine/schedule_create/schedule_delete）→ 放行（只动 Harness 状态；`cordis_run` 刻意不在其中） [allow]"]
    A8 --> A9["⑨ HARNESS_READ_TOOLS（14：job_output/job_list/schedule_list/list_subagent_models/session_search/cordis_inspect_*…）→ 放行 [allow]"]
