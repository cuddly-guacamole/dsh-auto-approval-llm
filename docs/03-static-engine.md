...
- **最严命中即胜**：同时命中多条时按 `deny` > `human` > `allow` 取最严，与声明顺序无关；同严重度并列保持声明序、取首条作为审计对象。`evaluateRules` 先用 `extractRuleTargets` 抽取动作面（command/script/code/prompt/text + file_path/path/cwd/workdir + content，后者仅在前者全空时兜底），防锚定正则（如 `^git push`）被 JSON 信封击穿；**`deny`/`human` 规则对整条动作面逐一匹配**（多一个无关字段不能把规则点名的值藏起来；因此**未加锚的规则也会命中文件内容** `content`——方向是 fail-closed，写规则时请用锚点收窄），**`allow` 规则只匹配主投影**（allow 是显式预授权，不得被无关字段的文本放宽）；取严意味着遍历全部声明规则（不短路），扫描保持规则数线性。
