...
| 文件 | 行 | 职责 |
|---|---|---|
| `constants.ts` | 16 | 数值阈值默认值**唯一事实源**：5/8/10s、3/20、4000、8s/1024 |
| `risk-tokens.ts` | 13 | HIGH 风险正则（NAME/REASON），供分类器与 policy 共用，防漂移 |
| `paths.ts` | 185 | 路径规范化（Windows 命名空间/NT 别名折叠、~ 展开、win32 小写）、受保护/关键路径判定 |
| `shell.ts` | 862 | Bash/PowerShell 词法分解（sticky 正则状态机）＋ 整行熔断 ＋ 逐段静态分类 |
| `policy.ts` | 330 | 每次工具调用的确定性第一遍分类 `assessTool`（推断型、保留类型检查） |
| `rules.ts` | 195 | Claude-Code 风格声明规则解析/求值（纯函数，host 与浏览器共用） |
| `classifier.ts` | 89 | 预分类提示词、参数脱敏、严格响应解析 |
| `dsh-classifier.ts` | 96 | 复用 `ctx.llm` 做低 token 分类请求（temperature 0） |
| `decision.ts` | 520 | 纯决策函数：评审解析、人机竞速、来源标注、熔断状态机、静态名单 |
| `trust.ts` | 69 | web 路由信任平面（loopback/LAN 边界、Host 伪造防护） |
| `artifacts.ts` | 57 | 本会话成功创建路径的内存出处登记（删除豁免依据） |
| `audit.ts` | 45 | append-only 审批审计（清空留墓碑、5MiB 裁剪） |
| `review-mode.ts` | 52 | 每会话评审模式持久化快照 |

## 3.1　assessTool —— 每一次调用的 17 步判定 <span class="lnum">policy.ts:L213-329</span>

下面 17 个分支与 `policy.ts:L213-329` 的判定顺序一一对应：
...
flowchart TD
    ...
    A3 --> A4["④ 读家族 read/read_image/grep/glob/lsp：无路径→放行；出工作区/受保护→交人工；否则放行 [read]"]
    A4 --> A5["⑤ write/edit：插件开发目录→放行；工作区外/受保护→交人工；常规→放行 [write]"]
    A5 --> A6["⑥ apply_patch：目标藏在 patches 各项的 file_path，任一不可读→ask（fail-closed）；全部常规→放行，否则交人工 [patch]"]
    A6 --> A7["⑦ str_replace_editor：command∈view/create/str_replace/insert 才认；view=读语义，其余=写语义 [editor]"]
    ...
    A16 --> A17["⑰ 兜底 = 普通未识别注册工具 → 放行（ordinary registered plugin tool） [allow]"]
```

## 3.2　硬拒闸门 hardDenyReason <span class="lnum">policy.ts:L181-211</span>
...
**symlink 逃逸**：文本层判定无法识破**快捷方式/软链接指向工作区外**（如 `ws/ln → ~/.bashrc`）。宿主侧守卫 `symlinkEscapeReason`（<span class="lnum">index.ts:L1389-1448</span>）取 `symlinkGuardTargets` 提取每个工具的真实路径操作数，对「文本上在工作区内/受信区」的目标做 realpath 最深祖先解析，一旦真实路径离开工作区/受信区就硬拒。
...
- 干跑 `rulesDryRun`：只记命中不执法（host 端 <span class="lnum">index.ts:L1822-1823</span>）。
