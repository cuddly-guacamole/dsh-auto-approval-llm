...
├─ 8 张可折叠子卡（均独立 保存/放弃；安全规则卡另有 恢复默认）
...
│    └─ 高级    defaultReviewMode · autoModeNotice · showSessionPanel（逐行即时保存，无独立 footer）· 仅配置文件可配的键（只读清单：按 `HOST_ONLY_KEYS` 单一 owner 逐键渲染「键名 + 当前生效值」，无控件、无保存路径；展开/收起由 `dsa-segBtn` 控制，计数写入文案 `{count}`）
