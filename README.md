...
- **读**优先规范目录；该副本不存在时回退到插件根目录的旧文件（已发布版本写入的位置）。规范副本存在时以它为准，旧副本永远不会遮蔽活数据。
