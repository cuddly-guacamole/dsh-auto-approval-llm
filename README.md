...
## 数据文件（规范位置 `<插件根目录>/runtime/`）

以下六个文件的**规范位置**是插件根目录下的 `runtime/` 子目录（插件写入前按需创建）；搬移前的旧位置就是插件根目录本身。

- **读**优先 `runtime/<文件名>`，该副本不存在时才回退到根目录的旧文件；两者都在时以 `runtime/` 为准——删除已搬移的文件不会被旧副本复活。
- **写**进 `runtime/`；目录创建失败（如插件根可写、但 `runtime/` 被同名文件占位）时回退到旧根路径并打印一条进程内一次性的 `console.warn`，而不是让写入失败。审计是 fail-closed 的提交闸（`appendAuditLine` 返回 false 会让裁决转拒），路径不可写不能变成「每条裁决都不可审计」。目录可用性缓存成功，但每次解析都会复检目录仍在——运行中被删掉时自动重建，避免「读回退旧根、写指向已删目录」。
- **保护不变**：六个文件仍在运行态保护名单里，该名单按**文件名**匹配，故移入子目录不削弱硬拒。
- **搬移**由用户在插件之外完成（插件自身的 guard 拒绝 agent 搬动自己的运行态文件）：停 dsh → 把六个文件移入 `runtime/` → 启动 dsh。该步骤可选——未搬移的安装靠读回退照常工作，只是旧副本仍留在插件根目录。仅追加型文件（history/audit/approval-debug/llm-latency）在首次写入前会把旧根内容原子复制进 `runtime/`，故旧记录不会因搬移而读不到。
