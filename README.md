...
- **读**优先规范目录；该副本不存在时回退到插件根目录的旧文件（已发布版本写入的位置）。规范目录可用时以规范副本为准，旧副本不会遮蔽活数据；**一旦写入降级到旧根，则两者中「较新」的那份为准**（新记录写向哪里，就读哪里：降级时规范内容会先被播种到旧根，下次启动规范目录恢复可用时再把较新的旧根副本搬回）。时间戳只在两份副本**经校验存在包含关系**时才被采信：仅追加型比字节前缀、覆盖型比条目包含，且**先校验后写入**；校验不过则读规范副本、保留两份并告警，不会仅凭时间戳做破坏性替换。
