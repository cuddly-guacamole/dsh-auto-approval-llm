...
- **读**优先规范目录；该副本不存在时回退到插件根目录的旧文件（已发布版本写入的位置）。规范目录可用时以规范副本为准，旧副本不会遮蔽活数据；**一旦写入降级到旧根，则两者中「较新」的那份为准**（新记录写向哪里，就读哪里：降级时规范内容会先被播种到旧根，下次启动规范目录恢复可用时再把较新的旧根副本搬回）。仅追加型在覆盖前**先校验字节前缀**（校验不过则读规范副本并告警，不凭时间戳做破坏性替换）；覆盖型是整份快照且会合法收缩（review-mode 不落默认值、learning 有 TTL/上限/撤销），故**较新者胜**，但若较新者不再包含规范副本的全部条目，被取代的副本会另存为 `<name>.superseded` 并告警。
- **写**进规范目录；目录无法创建、或**存在但拒绝写入**（启动期实写探针 + 写失败阶梯检出）时回退到插件根目录（同样打印进程内一次性的 `console.warn`），而不是让写入失败。审计是 fail-closed 的提交闸（`appendAuditLine` 返回 false 会让裁决转拒），路径不可写不能变成「每条裁决都不可审计」。降级只对「该位置拒绝写入」类错误（`EACCES`/`EPERM`/`EROFS`/`EISDIR`/`ENOTDIR`）生效并把决定粘滞；`ENOSPC` 这类字节已经动过的错误继续 fail-closed。目录可用性缓存成功，但每次解析都会复检目录仍在——运行中被删掉时自动重建，避免「读回退旧布局、写指向已删目录」。
- **自动迁移**：无需手工搬文件。仅追加型文件（history/audit/approval-debug/llm-latency）在首次写入前把旧位置内容经临时文件**原子复制**进规范目录；覆盖型（learning/review-mode）写出的本就是经读链加载后的整份内存状态。
- **保护更强**：规范目录位于 `DSH_HOME` 下，guard 对 `DSH_HOME` 的写入一律拒绝（不限于这六个文件名），读取仍落 `runtime-state-read` 观测事件。
- **迁移兼容代码有退役期限**：上述回退与自动迁移是为老安装准备的一次性过渡，**计划在本改动发布 3 个版本后（即版本号达到 0.0.25 时）移除**，只留规范目录。
