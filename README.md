...
- **写**进规范目录；目录无法创建时回退到插件根目录（同样打印进程内一次性的 `console.warn`），而不是让写入失败。审计是 fail-closed 的提交闸（`appendAuditLine` 返回 false 会让裁决转拒），路径不可写不能变成「每条裁决都不可审计」。目录可用性缓存成功，但每次解析都会复检目录仍在——运行中被删掉时自动重建，避免「读回退旧布局、写指向已删目录」。
